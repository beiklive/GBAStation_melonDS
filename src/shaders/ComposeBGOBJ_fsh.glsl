...
layout (std140, binding = 0) uniform ComposeUniform
{
    ...
    uint ThreeDScale, OutputScale, ThreeDLayerMask, _pad0;
    uvec4 Window[192];
};
...
void main()
{
    ...
    ivec2 nativePosition = position / int(max(OutputScale, 1U));
    ivec2 threeDPosition = OutputScale == ThreeDScale
        ? position
        : nativePosition * int(max(ThreeDScale, 1U));
    ...
    uint bgLayer0 = texelFetch(BGLayer0, (ThreeDLayerMask & 1U) != 0U ? threeDPosition : nativePosition, 0).r;
    uint bgLayer1 = texelFetch(BGLayer1, (ThreeDLayerMask & 2U) != 0U ? threeDPosition : nativePosition, 0).r;
    uint bgLayer2 = texelFetch(BGLayer2, (ThreeDLayerMask & 4U) != 0U ? threeDPosition : nativePosition, 0).r;
    uint bgLayer3 = texelFetch(BGLayer3, (ThreeDLayerMask & 8U) != 0U ? threeDPosition : nativePosition, 0).r;
